...
const debug = Utils.getLogger().debugContext('sql:websql');
...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    this.sql = AbstractQuery.formatBindParameters(sql, this.options.bind, this.options.dialect, { skipUnescape: true })[0];
        ...
        const executeSql = () => {
          if (this.sql.indexOf('-- ') === 0) {
            ...
            resolve(new Promise((resolve, reject) => {
              ...
              function errorCallback(transaction, err) {
                if (benchmark) {
                  query.sequelize.log('Executed: ' + query.sql, Date.now() - queryBegin, query.options);
                }
                err.sql = query.sql;
              ...
              db.executeSql(this.sql, parameters || [], successCallback, errorCallback);
