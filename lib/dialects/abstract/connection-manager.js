...
const setIntervalFn = window ? setInterval : eval("require('timers')").setInterval;
const clearIntervalFn = window ? clearInterval : eval("require('timers')").clearInterval;
  ...
  getConnection(options) {
    ...
    return promise.then(() =>
      new Promise((resolve, reject) => {
        ...
        const connectionTimer = setIntervalFn(() => {
          ...
          if (evictTimer) {
            clearIntervalFn(connectionTimer);
          }
        }, 0);
      })
    );
